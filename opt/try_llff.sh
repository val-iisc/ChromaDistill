...
if [[ ! -f "${ckpt_svox2}/ckpt.npz" ]]; then
    CUDA_VISIBLE_DEVICES=3 python opt.py -t ${ckpt_svox2} ${data_dir} \
                    -c configs/llff.json
fi

CUDA_VISIBLE_DEVICES=3 python opt_style.py -t ${ckpt_arf} ${data_dir} \
...
CUDA_VISIBLE_DEVICES=0 python render_imgs.py ${ckpt_arf}/ckpt.npz ${data_dir} \
